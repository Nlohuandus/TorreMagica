...
public class Juego extends InterfaceJuego {
	...
	Juego() {
		...
		// this.sprite=new Sprite(mago.getPosX(),mago.getPosY());arreglar esto. Subir
		// los archivos
	...
	public void dibujarPersonajes() {
		for (int i = 0; i < personajes.length; i++) {
			if (i == 0) {
				if (personajes[i].isEstado() == true) {
					// sprite.dibujar(entorno,mago.getPosX(),mago.getPosY(),mago.derecha,mago.mover);arreglar
					// esto, subir los archivos
					personajes[i].Dibujar(entorno);
					System.out.println(personajes[i].milisegundos);
				} else {
					personajes[i].Dibujar(entorno, Color.green);
				}
			} else {
